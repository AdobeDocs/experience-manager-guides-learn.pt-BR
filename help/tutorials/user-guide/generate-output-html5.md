...
| Opções de HTML5 | Descrição |
| --- | --- |
| Tipo de saída | O tipo de saída que você deseja gerar. Para gerar saída de HTML5, escolha a opção HTML5. |
| Nome da configuração | Dê um nome descritivo para as configurações de saída do HTML5 que você está criando. Por exemplo, você pode especificar _Saída de clientes internos_ ou _saída de usuários finais_. |
...
| Nome do arquivo | Especifique o nome do arquivo com o qual deseja salvar a saída do HTML5.<br><br>**Observação**: se você não fornecer um nome de arquivo, o título do mapa DITA será usado para gerar o nome final do arquivo de saída HTML5. Se o mapa não tiver um título, o nome de arquivo do mapa DITA será usado para nomear a saída HTML5 final. O nome do arquivo é limpo usando as regras configuradas no sistema para lidar com qualquer caractere inválido. |
